...
while True:
    try:
        ...
        account_being_followed = account_being_followed + following_query_result['data']
...
# Get the list fo BLOCKED accounts
accounts_being_blocked = []
pagination_token = None
while True:
    try:
        blocked_query_result = api.get_blocking_users(user_id=my_twitter_id,
                                                      return_json=True,
                                                      pagination_token=pagination_token)

        jsonschema.validate(blocked_query_result, followers_json_schema)

        accounts_being_blocked = accounts_being_blocked + blocked_query_result['data']

        if 'next_token' not in blocked_query_result['meta'].keys():
            break

        pagination_token = blocked_query_result['meta']['next_token']
        ...
        logging.info(f"Blocked Twitter user rate limit exceeded. Waiting {follows_rate_limit_window_seconds} seconds.")
        time.sleep(follows_rate_limit_window_seconds)
        continue
    except pytwitter.error.PyTwitterError as ptw:
        logging.fatal("PyTwitterError with unknown message format '{}'".format(ptw.message['status'], ptw.message))
...
# Collect all of the user's lists
owned_lists = []
pagination_token = None
with open("twitter_lists_endpoint_response_schema.json") as response_schema_file:
    list_json_schema = json.load(response_schema_file)

while True:
    try:
        list_query_result = api.get_user_owned_lists(user_id=my_twitter_id,
                                                     return_json=True,
                                                     pagination_token=pagination_token)

        jsonschema.validate(list_query_result, list_json_schema)

        owned_lists = owned_lists + list_query_result['data']
        ...
        logging.info(f"Twitter user list rate limit exceeded. Waiting {follows_rate_limit_window_seconds} seconds.")
        time.sleep(follows_rate_limit_window_seconds)
        continue
    except pytwitter.error.PyTwitterError as ptw:
        logging.fatal("PyTwitterError with unknown message format '{}'".format(ptw.message['status'], ptw.message))
...
# For owned list, get the members (not followers) of the list
...
for owned_list in owned_lists:
    ...
    while True:
        try:
            list_follows_result = api.get_list_members(list_id=owned_list["id"],
                                                       return_json=True,
                                                       pagination_token=pagination_token)

            jsonschema.validate(list_follows_result, followers_json_schema)

            owned_list["follows"] = owned_list["follows"] + list_follows_result["data"]

            if 'next_token' not in list_follows_result['meta'].keys():
                break

            pagination_token = list_follows_result['meta']['next_token']
            ...
            logging.info(f"Twitter list follows rate limit exceeded. Waiting {follows_rate_limit_window_seconds} seconds.")
            time.sleep(follows_rate_limit_window_seconds)
            continue
        except pytwitter.error.PyTwitterError as ptw:
            logging.fatal("PyTwitterError with unknown message format '{}'".format(ptw.message['status'], ptw.message))
...
archive["FOLLOWING"] = account_being_followed
archive["BLOCKING"] = accounts_being_blocked
archive["LISTS"] = owned_lists

timestamp_str = arrow.utcnow().format('YYYY-MM-DD_HH:mm:ss_ZZ')

archive_file_name = f'{twitter_me["data"]["username"]}_twitter_archive_{timestamp_str}.json'

with open(archive_file_name, 'w') as handle:
    json.dump(archive, handle)

print(f"Archived to {archive_file_name}")
